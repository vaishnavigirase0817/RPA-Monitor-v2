...
export const MobileDrawer = ({ isOpen, onClose, activeTab, onNavigate }: MobileDrawerProps) => {
  const tabs: { id: TabId; icon: React.ReactNode }[] = [
    ...
    {
      id: 'Analytics',
      icon: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M10.5 6a7.5 7.5 0 107.5 7.5h-7.5V6z" /><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 10.5H21A7.5 7.5 0 0013.5 3v7.5z" /></svg>
    },
    {
      id: 'Activity Logs',
  ...
  return (
    <div className="fixed inset-0 z-[100] flex">
      {/* Backdrop */}
      <div 
        className="absolute inset-0 bg-black/70 backdrop-blur-sm transition-opacity animate-[fadeIn_0.2s]"
        onClick={onClose}
        aria-hidden="true"
      />
      
      {/* Drawer */}
      <div className="relative w-72 h-full bg-navy border-r border-white/10 shadow-2xl flex flex-col animate-[slideInLeft_0.3s_ease-out]">
        <div className="h-14 flex items-center px-6 border-b border-white/5 bg-black/20 shrink-0">
          ...
          <button 
            onClick={onClose}
            className="ml-auto text-gray-400 hover:text-white p-1 rounded hover:bg-white/10 transition-colors"
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all group ${
                  isActive 
                    ? 'bg-electric-blue/10 text-electric-blue border border-electric-blue/20 shadow-[0_0_15px_rgba(0,255,255,0.1)]' 
                    : 'text-gray-400 hover:text-white hover:bg-white/5 border border-transparent'
                }`}
              >
                <div className={`transition-transform duration-300 ${isActive ? 'scale-110' : 'group-hover:scale-110 group-hover:text-electric-blue/70'}`}>
                  {tab.icon}
                </div>
                {tab.id}
              </button>
